...
# Go to the upper right R Studio window, in the Workspace tab, click on iris.data. It will open
...
# There are two primary ways of accessing different colums of data in our data.frames.  One method is by using matrix notation where we input the numerical position for the row and column, seperated by a comma.  If we wanted to print the 10th row and the 7th column, we would type iris.data[10,7].
...
# The other method of accessing entire columns of data is to use the dollar sign, $, and the name of the column we want to access.  So to print the column for Species, we would type iris.data$Species
...
# From here we can access particular rows in the iris.data$Species vector by using the numerical method as described earlier, only now we don't need to use any commas since the iris.data$Species vector is one-dimensional whereas the data.frame is two-dimensional.  So, let's print the 10th entry in the iris.data$Species vector.
...
# If we wanted to print more than one row from our iris.data$Species vector (or more than one column and row from our iris.data data frame) then we can use the c() command and the colon symbol for listing.  For example, let's print rows 1 through 10 in the iris.data$Petal.Width column of data by indexing with c(1:10).
...
# You may have noticed that the commands iris.data$Petal.Width[10] and iris.data$Species[10] returned slightly different results.





# iris.data$Petal.Width[10] returned "[1] 0.1", while iris.data$Species[10] returned "[1] setosa
# Levels: setosa versicolor virginica".  That is, iris.data$Petal.Width[10] just returned the numerical value, and iris.data$Species[10] returned both the Species value as well as a list of all of the other Speciess available.  This is because these two columns are different classes, or types of data.  The main classes that we will be working with are:
...
# Now let's do some very rudimentary statistics by calculating the mean value of sepal.length and adding its value to the graph as a line.  First we wil use the mean function.
...
## We want to extract all of the rows of the iris.data data frame whose species value is either setosa, versicolor, or virginica.  We can do this by writing a logical statement comparing every value in the species vector with the name of interest.
...
## To keep things tidy, we will define a new data frame for just these mean values
...
##### Now, for the rest of the day, produce a similar graph as above but for the hawks_day1.csv file.  Use the variable "Tail" for your histogram data, and label the "Species Name" legend with the full species names (CH = Cooper's Hawk, RT = Red Tail, SS = Sharp Shinned), and present the mean values of the tails for each species in the legend as we did for the flowers, as well as vertical dashed lines demonstrating their values.
...
## Tomorrow, we will look at different kinds of graphs, statistical functions, and some more advanced programming methods.
